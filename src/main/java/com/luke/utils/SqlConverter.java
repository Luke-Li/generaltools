...
public class SqlConverter {
	...
	public static Object convert(ResultSet rs, Class cls)
		...
		for (int i = 0; i < rs.getMetaData().getColumnCount(); i++) {
			...
			if (setMethod != null) {
				Object[] params = null;
				if (rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("NUMERIC")
						|| rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("DECIMAL")
						|| rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("DOUBLE")) {
					if (rs.getMetaData().getScale(i + 1) > 0)
						params = new Object[] { rs.getDouble(i + 1) };
					else if (rs.getMetaData().getPrecision(i + 1) > 9)
				...
				} else if (rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("INT")
						|| rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("INTEGER")) {
					params = new Object[] { rs.getInt(i + 1) };
				} else if (rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("BIGINT")) {
					params = new Object[] { rs.getLong(i + 1) };
				} else if (rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("CHAR")
						|| rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("VARCHAR")
						|| rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("TEXT")
						|| rs.getMetaData().getColumnTypeName(i + 1).equalsIgnoreCase("XML")) {
